if game.CoreGui:FindFirstChild("See-nuan Hub") then
    game.CoreGui:FindFirstChild("See-nuan Hub"):Destroy()
end
...
local Library = {
    ...
    AccentColor = Color3.fromRGB(131, 255, 97);
...
local Window = Library:CreateWindow('Japenjonsaladhaidailaey - [UPDATE 20] Blox Fruits')

local Tabs = {
    Captain = Window:AddTab('Captain'), 
    Crew = Window:AddTab('Crew'), 
}
...
Settings:AddDropdown('ChooseaCaptain', {
    Values = Playerslist,
    Default = _G.ChooseaCaptain,
    Multi = false,
    Text = 'Choose a Captain',
})
...
Options.ChooseaCaptain:OnChanged(function()
    _G.ChooseaCaptain = Options.ChooseaCaptain.Value
    ChooseaCaptains = Options.ChooseaCaptain.Value
    CheckCaptain()
    ...
    Options.ChooseaCaptain:SetValue()
end)
...
function test()
    print(_G.ChooseaCaptain)
end

Settings:AddButton('Test', function()
    test()
end)
...
Toggles.AutoParry:OnChanged(function()
    _G.AutoParry = Toggles.AutoParry.Value
end)

local Distancex = Main:AddLabel('Distance : N/A')

Main:AddSlider('Distance', {
    Text = '',
    Default = 10,
    Min = 1,
    Max = 20,
